...
본 프로젝트는 확장 가능한 웹 기반 3D 물리 시뮬레이터를 개발하는 것을 목표로 합니다.
...
- **렌더링 최적화**:
  ...
  - 궤적 라인: 단색 선으로 표현
  - **목적**: 물리 현상 시각화에 집중, 사실적 렌더링보다 명확한 움직임 강조
- 번들 크기 < 2MB (gzip)
...
1.4.3 UI/UX 제약
- 블렌더 FBX 투수 모델 구현 (846KB, 119프레임 애니메이션, 48프레임 릴리스 동기화)
- 좌완/우완 Y축 미러링 자동 적용 (`releasePoint.x < 0` 기준)
...
interface ForceVectors3DProps {
  ...
  scale?: number              // 화살표 크기 스케일 (기본값: 0.1)
  experimentId?: string       // 비교 모드에서 실험 구분 (A/B)
}


출력: Three.js <arrowHelper> JSX 요소 3개 (중력-빨강, 항력-파랑, 마그누스-초록)

**주요 기능**:
- 3D 화살표로 힘 벡터 시각화 (중력/항력/마그누스)
- 리플레이 시점에 따라 실시간 업데이트
- 비교 모드에서 실험 A/B 독립적으로 표시
- 결과 패널에서 토글로 표시/숨김 제어

**연동 컴포넌트**:
- ResultPanel: "힘 벡터 표시" 체크박스 + 수치 표 (X/Y/Z 성분 + 크기)
- ComparisonPanel: "힘 벡터 비교" 체크박스 + 비교 표 (A/B + 차이값)
...
**자동 보정 시스템**:
- 릴리스 포인트에서 홈플레이트 중심(X=0, Z=-18.44)으로 향하는 자연 각도 자동 계산
- `naturalHorizontalAngle = atan2(deltaX, -deltaZ)`
- 사용자 입력 horizontal angle을 자연 각도에 더하여 미세 조정 가능
- 우완/좌완 투수가 바깥쪽 릴리스 포인트에서도 홈플레이트 중심을 향하도록 자동 보정
...
- **시작하기 탭**:
  ...
  - 3단계: 결과 확인 방법
- **키보드 단축키 탭**:
  - 시뮬레이션 제어: Space (재생/일시정지), R (처음으로 되돌리기), Esc (일시정지)
  - 카메라 프리셋: 1 (포수), 2 (측면), 3 (투수), 4 (추적), 5 (자유)
  - 리플레이 제어: ←→ (0.1초 이동, 일시정지 중에도 작동), [] (속도 조절 0.25x~2x), ,. (속도 0.5x/1x)
- **FAQ 탭**:
  - 스트라이크 존 정의 및 판정 기준
  - 회전축 (X/Y/Z) 효과 설명
  - 환경 변수 영향 설명
...
3.4.11 GraphicsSettingsPanel (그래픽 설정 패널)

**모듈 이름**: GraphicsSettingsPanel

**모듈 형**: React 함수형 컴포넌트

**인터페이스**:
```typescript
// GraphicsContext에서 제공하는 설정
interface GraphicsSettings {
  targetFps: number          // 목표 FPS (30-60)
  dpr: number                // 픽셀 밀도 (1.0-2.0)
  ballSegments: number       // 공 폴리곤 수 (8-32)
  trajectoryPoints: number   // 궤적 정밀도 (50-200)
...
**주요 기능**:
- **프리셋 가이드라인**: 3가지 권장 설정
  - 저사양: 30fps, DPR 1.0, 공 8면, 궤적 50점
  - 중간: 45fps, DPR 1.5, 공 16면, 궤적 100점
  - 고사양: 60fps, DPR 2.0, 공 32면, 궤적 200점
- **자유 조정 설정**: 각 항목별 슬라이더/토글 제공
- **실시간 적용**: 설정 변경 시 즉시 렌더링 반영
- **GraphicsContext 연동**: 전역 상태 관리

**사용하는 파일**:
| 파일명 | 용도 |
|--------|------|
| @/contexts/GraphicsContext | 그래픽 설정 전역 상태 |
...
| 모듈명 | 호출 시점 | 목적 |
|--------|----------|------|
| Scene3D | 설정 변경 시 | DPR, FOV, 안티앨리어싱 적용 |
| Ball3D | 설정 변경 시 | 공 폴리곤 수 동적 조정 |
| PitchSimulator | 설정 변경 시 | 목표 FPS, 궤적 정밀도 적용 |

**기능 설명**:

성능 영향도:
- 픽셀 밀도(DPR): 가장 큰 영향 (렌더링 해상도)
- 공 폴리곤 수: 중간 영향 (지오메트리 복잡도)
- 궤적 정밀도: 중간 영향 (라인 렌더링)
- 안티앨리어싱: 작은 영향 (후처리)
- 시야각(FOV): 거의 없음 (시야 범위만 변경)

UI 레이아웃:
- 프리셋 버튼 3개 (수평 배치)
- 구분선
- 각 설정 항목: 라벨 + 슬라이더/토글 + 현재 값 표시
- 스크롤 가능한 컨테이너

실시간 반영:
- Scene3D: `setPixelRatio(dpr)`, `camera.fov = fov`, `antialias` 재초기화 필요
- Ball3D: `new THREE.SphereGeometry(radius, ballSegments, ballSegments)`
- 애니메이션: `targetFps`에 따라 프레임 간격 조정
- 물리 계산: `trajectoryDt = 1 / trajectoryPoints`
...
**개선 사항**:
- 단일 시간 기준 통합: `animationIndex`, `isAnimating` 제거 → `replayTime` 단일 기준 사용
- 정확한 일시정지: 슬라이더/스페이스바 사용 시 현재 위치에서 정확히 정지
- 슬라이더 조작 시 `replayTime`과 자동 동기화
...
3.9.1 모듈 이름ComparisonPanel + ComparisonContext (시뮬레이션 비교 기능)

3.9.2 모듈 형React 함수형 컴포넌트 + Context API
...
3.10.7 기능 설명
**설계 배경:**
"파라미터 변화가 결과에 어떤 영향을 주는가?"를 직관적으로 분석하기 위해서는
...
6. 결과 비교 테이블 표시:
  ...
  - 수직 낙차 (차이: cm)
  - 통과 높이 (차이: cm)
7. **[신규]** "힘 벡터 비교" 체크박스 클릭
  - 타임라인 슬라이더로 시점 선택
  - 3D 뷰: 실험 A/B의 힘 벡터 화살표 동시 표시
  - 비교 표: 중력/항력/마그누스 각 3개 힘별로 X/Y/Z 성분 + 크기 + 차이값(A-B) 표시
8. "비교 종료" 버튼으로 일반 모드 복귀**
3D 궤적 렌더링 구현:
**typescript
...
**구현 상태:**
- ✅ ComparisonContext 구현 완료
- ✅ ComparisonPanel UI 구현 완료 (2개 선택 + 미리보기)
- ✅ ComparisonResultTable 구현 완료 (6개 비교 지표)
- ✅ 2개 궤적 동시 렌더링 구현 완료 (파랑/빨강)
- ✅ **[신규]** ComparisonForceVectorTable 구현 완료 (힘 벡터 비교)
- ✅ **[신규]** 비교 모드 3D 힘 벡터 시각화 (실험 A/B 독립 표시)
- ✅ **[신규]** 타임라인 슬라이더 연동 (리플레이 시점 선택)
- ✅ 비교 시작/종료 버튼 구현 완료
