...
7. 9가지 구종 프리셋
   - 직구 (Fastball), 투심 (Two-Seam), 커브 (Curveball)
   - 슬라이더 (Slider), 커터 (Cutter), 체인지업 (Changeup)
   - 싱커 (Sinker), 포크볼 (Forkball), 너클볼 (Knuckleball)
...
React 컴포넌트에서 SimulationContext를 거쳐 물리 엔진, 그리고 백엔드 API와 PostgreSQL까지의 계층적 구조를 보여줍니다.
...
파일 위치: mermaid/1-architecture.mmd
설명: 사용자, 웹 브라우저, 프론트엔드, 백엔드, 데이터베이스 간의 상호작용을 표현합니다.
...
│   │   ├── simulation.routes.ts # /api/simulations/*
│   │   └── minigame.routes.ts   # /api/minigame/*
│   ├── controllers/          # 요청 핸들러
│   │   ├── auth.controller.ts
...
[다이어그램 참조: 백엔드 디렉토리 구조]
파일 위치: mermaid/6-backend-structure.mmd
설명: 백엔드 디렉토리 구조 및 모듈 간의 의존성을 표현합니다.
features (기능별 모듈), middleware (미들웨어), prisma (데이터베이스 스키마)의 구조를 보여줍니다.
...
2. **투구 조건 파라미터**: 초기 속도, 발사각(수평/수직), 회전수(ω, rpm)
...
3.5.1 모듈 이름
auth.service (백엔드 인증 서비스)

3.5.2 모듈 형
TypeScript 클래스 (Express.js 서비스 레이어)
...
3.6.1 모듈 이름
simulations.service (시뮬레이션 저장/조회 서비스)
...
class SimulationsService {
  async saveSimulation(userId: string, data: SaveSimulationDto): Promise<Simulation>
  async getMySimulations(userId: string): Promise<Simulation[]>
  async getSimulationById(id: string): Promise<Simulation>
  async deleteSimulation(userId: string, id: string): Promise<void>
  async shareSimulation(id: string): Promise<{ shareToken: string }>
  async getSharedSimulation(shareToken: string): Promise<Simulation>
}
...
interface SaveSimulationDto {
  scenarioType: 'pitch' | 'projectile' | ...
  parameters: Record<string, any>           // JSON
  result: Record<string, any>               // JSON
}
...
| 오류 코드 | 메시지 | HTTP 상태 |
|---|---|---|
| SIM_SAVE_001 | "저장 한도 초과 (최대 10개)" | 400 Bad Request |
| SIM_SAVE_002 | "존재하지 않는 시뮬레이션입니다" | 404 Not Found |
| SIM_SAVE_003 | "권한이 없습니다" | 403 Forbidden |
| SIM_SAVE_004 | "공유 토큰이 만료되었습니다" | 410 Gone |
...
|---|---|
| prisma/client | 데이터베이스 접근 |
| crypto | 공유 토큰 생성 (랜덤 문자열) |

3.6.6 호출하는 모듈

| 모듈명 | 호출 메서드 | 목적 |
|---|---|---|
| prisma.simulation | create() | 시뮬레이션 저장 |
| prisma.simulation | findMany() | 내 시뮬레이션 목록 조회 |
| prisma.simulation | findUnique() | 특정 시뮬레이션 조회 |
| prisma.simulation | delete() | 시뮬레이션 삭제 |
| crypto | randomBytes() | 공유 토큰 생성 |

3.6.7 기능 설명

설계 배경:
시나리오마다 파라미터 구조가 다릅니다. PitchParameters는 17개 필드를 가지지만, ProjectileParameters는 5개 필드만 가집니다. 각 시나리오마다 별도의 테이블을 만들면 테이블 수가 증가하고, 공통 로직 작성이 어렵습니다.

해결 방안:
Simulation 테이블의 parameters와 result 컬럼을 JSONB 타입으로 정의하여 유연한 스키마를 지원합니다. 새로운 시나리오가 추가되어도 테이블 구조를 변경할 필요 없이, scenarioType 필드로 구분하여 저장할 수 있습니다.

저장 제한 정책:
- 사용자당 최대 10개 시뮬레이션 저장 가능
- 초과 시 가장 오래된 것 자동 삭제 (트랜잭션으로 원자적 처리)
- 이유: 데이터베이스 스토리지 비용 절감 및 무분별한 저장 방지

공유 기능 구현:
1. shareSimulation(id) 호출
2. 32바이트 랜덤 토큰 생성 (crypto.randomBytes(32).toString('hex'))
3. DB에 shareToken + shareExpiresAt (30일 후) 저장
4. 공유 URL 생성: https://domain.com/shared/{shareToken}

보안 고려사항:
- 공유 토큰은 추측 불가능한 256비트 랜덤 문자열 사용
- 30일 후 자동 만료로 영구 공유 방지
- 공유 시뮬레이션은 읽기 전용 모드로만 접근 가능
...
3.10.1 모듈 이름
ComparisonMode (시뮬레이션 비교 기능)

3.10.2 모듈 형
React 함수형 컴포넌트 + Context API
...
interface ComparisonModeProps {
  isEnabled: boolean                    // 비교 모드 활성화 여부
  onToggle: () => void                  // 모드 토글 핸들러
}

interface ComparisonData {
  id: string                            // 저장 데이터 ID
  name: string                          // 사용자 지정 이름
  parameters: PitchParameters           // 시뮬레이션 파라미터
  result: SimulationResult              // 시뮬레이션 결과
  createdAt: Date                       // 저장 일시
}

interface EnvironmentSyncOption {
  mode: 'independent' | 'unified'       // 환경 변수 설정 모드
  baseDataId?: string                   // 통일 시 기준 데이터 ID
}
```
...
| 오류 코드 | 메시지 | 발생 조건 |
|---|---|---|
| CMP_001 | "저장된 데이터가 없습니다. 먼저 시뮬레이션을 저장하세요" | 저장 데이터 0개 |
| CMP_002 | "비교하려면 최소 2개의 저장 데이터가 필요합니다" | 저장 데이터 1개 |
| CMP_003 | "이미 선택한 데이터입니다" | 중복 선택 시도 |
| CMP_004 | "선택한 데이터가 삭제되었습니다" | 삭제된 데이터 선택 |
| CMP_005 | "시나리오 타입이 일치하지 않습니다" | 다른 시나리오 데이터 선택 |

3.10.5 사용하는 파일

| 파일명 | 용도 |
|---|---|
| simulation.service | 저장 데이터 CRUD |
| PitchSimulator | 시뮬레이션 실행 |
| Scene3D | 2개 궤적 동시 렌더링 |
| ReplayControls | 동기화된 리플레이 제어 |

3.10.6 호출하는 모듈

| 모듈명 | 호출 메서드 | 목적 |
|---|---|---|
| simulations.service | GET /api/simulations | 저장 목록 조회 |
| simulations.service | GET /api/simulations/:id | 특정 데이터 로드 |
| PitchSimulator | simulate() | 2개 시뮬레이션 실행 |

3.10.7 기능 설명

**설계 배경**:
교육 현장에서 "파라미터 변화가 결과에 어떤 영향을 주는가?"를 직관적으로 학습하기 위해서는 2개 시뮬레이션을 동시에 비교할 수 있어야 합니다. 예를 들어, 스핀 속도만 다른 두 투구를 비교하면 마그누스 효과의 영향을 명확히 이해할 수 있습니다.

**사용자 플로우**:
1. 사용자가 시뮬레이션 실행 후 "저장" 버튼 클릭
2. 이름 입력 모달 표시 → 저장 (최대 10개)
3. "비교 모드" 토글 활성화
4. "비교할 데이터 1" 버튼 클릭 → 저장 목록 모달 표시 → 선택
5. "비교할 데이터 2" 버튼 클릭 → 저장 목록 모달 표시 → 선택 (이미 선택된 데이터는 비활성화)
6. 환경 변수 설정 선택:
   - "각자 저장된 환경 사용" (기본): 환경 차이 비교
   - "환경 변수 통일": 순수 투구 파라미터 차이만 비교
7. "비교 시작" 버튼 활성화 → 클릭
8. 2개 시뮬레이션 동시 실행 (Promise.all)
9. 3D 렌더링: 궤적1(파랑), 궤적2(빨강)
10. 계측값 표 표시:
    - 필터: 시간별 (0.0s, 0.1s, ...) / 거리별 (0m, 5m, 10m, ...)
    - 항목: 위치, 속도, 회전수, 힘 벡터
    - 차이값 표시: Δv = +2.3 m/s
11. 리플레이 동기화: 두 궤적 동시 재생, 속도 조절 (0.5x ~ 2x)

**환경 변수 처리**:
```typescript
// 독립 모드
const result1 = await simulator1.simulate(data1.parameters)
const result2 = await simulator2.simulate(data2.parameters)

// 통일 모드 (데이터1 환경 기준)
const params2Modified = {
  ...data2.parameters,
  environment: data1.parameters.environment  // 환경 교체
}
const result2 = await simulator2.simulate(params2Modified)
```

**방어 로직**:
- DB 조회 시 `scenarioType: 'pitch'` 필터링
- 선택 전 데이터 존재 여부 재확인
- 중복 선택 UI 레벨 차단
- 저장 데이터 개수 검증 (0개/1개 → 에러)
- 궤적 길이 불일치: 짧은 쪽 기준 계측표 생성
- 버전 마이그레이션: `parameters.version` 체크

**데이터 저장 구조** (기존 Simulation 테이블 활용):
```typescript
// Simulation 테이블 (변경 없음)
{
  id: UUID
  userId: UUID
  scenarioType: 'pitch'
  parameters: JSONB  // 3단 구조 (ball, initial, environment)
  result: JSONB      // trajectory, metadata
  name: VARCHAR(100) // 신규 필드 (선택적)
  createdAt: TIMESTAMP
}
```

**성능 최적화**:
- 2개 궤적 동시 렌더링 시 LOD (Level of Detail) 적용
- 리플레이 일시정지 시 렌더링 루프 중단
- 저사양 PC: 자동으로 30fps 제한

**교육적 가치**:
- 단일 변수 영향 분석 (예: 스핀만 변경)
- 환경 조건 영향 학습 (예: 중력 차이)
- 이론-실제 비교 (예: 공식 계산값 vs 시뮬레이션)
...
JSONB 컬럼 사용 이유:
- parameters: 투구는 17개 필드, 포물선은 5개 필드 등 시나리오마다 구조가 다름
...
| FR-002 | 전문가 모드 파라미터 조정 | GenericInputPanel (메타데이터 확장) |  완료 | 17개 파라미터 세밀 조정 |
