...
1. **타입 체크 실행**: `npx tsc --noEmit` (마지막 작업 중단으로 미확인)
2. **회전축 동작 테스트** (중요!):
   - spinY=+5000 → 1루 방향 확인
   - spinY=-5000 → 3루 방향 확인
...
- **브랜치**: `dev` (현재 작업 중)
- **타입 체크**: ⚠️ 미확인 (마지막 작업 중단)
- **빌드**: 미확인
- **최근 작업**: 릴리스 포인트 보정 + 회전축 수정 + 투수 모델 (2025-10-30)
- **주요 기능**:
  - ✅ 로그인/회원가입
  - ✅ 실험 저장/불러오기 (Supabase)
...
1. **타입 체크 실행**: `npx tsc --noEmit` (마지막 작업 중단으로 미확인)
2. **회전축 동작 테스트** (중요! Breaking Change):
   - spinY=+5000 → 1루 방향 확인
   - spinY=-5000 → 3루 방향 확인
   - spinX=+2400 → 백스핀 떠오름 확인
   - 릴리스 포인트 X=+0.5, horizontal=0 → 홈플레이트 중심 통과 확인
3. **투수 모델 확인**:
   - 오버핸드(releaseY=2.0), 사이드암(1.7), 언더핸드(1.2) 폼 변화
   - 우완/좌완 구분 (releaseX 부호)
   - 투구 전 공 표시, 릴리스 후 숨김
4. **스트라이크 존 판정**: 경계에 걸친 공이 스트라이크 판정되는지 확인
